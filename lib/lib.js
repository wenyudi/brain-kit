...
// 实例开关（各库 _system/vault.json；kit 各脚本共读，没有就全默认）
let VCFG = {};
...
const CONTRACT = {
  ...
  // 退役顶层目录（写时拦 + 同步推尸哨兵）：各库历史不同，由 vault.json `retired` 定；kit 默认空
  RETIRED: [],
  DD: /^\d{4}-\d{2}-\d{2}$/,
  WEEK: /^\d{4}-W\d{2}$/,
  ...
  ASR: { model: 'medium' },
  // 藏书房图片重试：夜间 ingest 自动重试这么多次仍失败就标 assets_dead（签名 URL 过期/图床下线），lint 不再天天报
  ASSETS_MAX_ATTEMPTS: 3,
};
const retiredDirs = () => (Array.isArray(VCFG.retired) ? VCFG.retired : CONTRACT.RETIRED);
...
// obsidian 可执行文件：systemd 里 PATH 没有 ~/.local/bin，裸调恒失败 → 退回 fs 删 → 同步插件推尸（审计 2026-09-03）
let _obsBin;
function obsidianBin() {
  if (_obsBin !== undefined) return _obsBin;
  const { spawnSync } = require('child_process'); const os = require('os');
  _obsBin = ['obsidian', path.join(os.homedir(), '.local', 'bin', 'obsidian'), '/usr/local/bin/obsidian']
    .find(c => { try { return spawnSync(c, ['vaults'], { stdio: 'ignore', timeout: 5e3 }).status === 0; } catch { return false; } }) || null;
  return _obsBin;
}
function obsidianCli(args, opts = {}) {
  const { spawnSync } = require('child_process');
  const vault = opts.vault || VCFG.obsidianVault || path.basename(ROOT);   // Obsidian 里的库名 ≠ 目录名时在 vault.json 填 obsidianVault
  const bin = obsidianBin(); if (!bin) return { ok: false, out: '', err: 'obsidian CLI not found or app not running' };
  try {
    const r = spawnSync(bin, [`vault=${vault}`, ...args], { encoding: 'utf8', timeout: opts.timeout || 15e3 });
...
function vaultDelete(absFile) {
  ...
  if (obsidianUp()) {
    const r = obsidianCli(['delete', `path=${rel}`]);
    if (r.ok) {   // app 侧删除可能是异步的：等一小会儿再判断，别急着退回 fs 删
      const until = Date.now() + 3000;
      while (fs.existsSync(absFile) && Date.now() < until) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, 200);
      if (!fs.existsSync(absFile)) return 'obsidian';
    }
  }
  try { fs.unlinkSync(absFile); } catch (e) { if (e.code !== 'ENOENT') throw e; }
  return 'fs';
}
