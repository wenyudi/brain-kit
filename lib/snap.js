...
// 实例自己的人读层目录（vault.json humanDirs，如 Work Vault 的「速查」）同样是合法顶层——不然库外写进去的新文件会被当陌生路径漏掉（2026-09-03）
for (const d of [...(VCFG.humanDirs || []), 'AGENTS.md']) ALLOW.add(d);
...
try {
  ...
  if (!busy && /^(master|main)$/.test(git('symbolic-ref', '--short', 'HEAD').trim())) {
    ...
    // 实例可选 Stop 即推（vault.json checkpoint.push；livesync 在跑的库要远端快）：只在有未推提交时推，20 s 超时，失败静默
    if (mode === 'checkpoint' && VCFG.checkpoint && VCFG.checkpoint.push) {
      let ahead = 0; try { ahead = +git('rev-list', '--count', '@{u}..HEAD').trim(); } catch { /* 无上游 */ }
      if (ahead > 0) { try { execFileSync('git', ['push', '-q'], { cwd: ROOT, encoding: 'utf8', timeout: 20000, stdio: 'ignore' }); } catch { /* 离线，快照兜 */ } }
    }
  }
