...
// source: obsidian 的手记不动（会话里 LLM 分流）。用法: node ingest.js [--dry-run] [--no-asr] [--no-commit] | --repair-assets（对 assets_failed 的 raw 重试图片，只改链接）
...
async function ingestOne(file, existingUrls) {
  ...
  if (!note) {
    ...
    if (!dry) fs.writeFileSync(stubFile, matter.stringify(sbody, sfm));
  }
...
async function repairAssets() {
  ...
  for (const f of listMdIn(LIB_ROOT, 'raw')) {
    const n = readNote(f); if (!(+n.data.assets_failed > 0)) continue;
    const base = path.basename(f, '.md');
    log(`🔧 重试图片：${base}（上次失败 ${n.data.assets_failed}）`);
    const img = await localizeImages(n.content, n.data.url, base);
    if (!img.ok) { log('   仍全部失败'); continue; }
    const fm = { ...n.data, assets: (+n.data.assets || 0) + img.ok };
    if (img.failed) fm.assets_failed = img.failed; else delete fm.assets_failed;
    fm.assets_repaired = today;
    if (!dry) fs.writeFileSync(f, matter.stringify(img.body, fm));
    touched++; log(`   补回 ${img.ok}${img.failed ? `，仍失败 ${img.failed}` : ''}`);
  }
  if (touched && !dry && !noCommit) { try { execFileSync('git', ['add', '-A', '--', '_ai/library'], { cwd: ROOT, stdio: 'ignore' }); execFileSync('git', ['commit', '-q', '-m', `ingest: repair assets on ${touched} raw (links → local copies, text unchanged)`], { cwd: ROOT, stdio: 'ignore' }); } catch { /* 快照兜 */ } }
  log(`${dry ? '[dry-run] ' : ''}修补 ${touched} 篇`);
}

(async () => {
  if (args.includes('--repair-assets')) return repairAssets();
  const files = listMdIn(ROOT, 'inbox');
  if (!files.length) { log('inbox 空'); return; }
