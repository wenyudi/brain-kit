...
const RULES = [
  ...
  // 各家 CLI 的登录态/凭据文件(审计 2026-09-03 补):Claude/Codex OAuth、gh、kube、docker、netrc、direnv
  { name: 'CLI 凭据文件(.credentials.json/auth.json/gh hosts.yml/kube/docker)', re: /\.claude\/\.credentials\.json|\.codex\/auth\.json|\.config\/gh\/hosts\.yml|\.kube\/config\b|\.docker\/config\.json/ },
  { name: 'netrc/envrc', re: /(?:^|[\/\s'"=(])\.(?:netrc|envrc)\b/ },
  // `cd ~ && cat Private/key.md` 这种相对写法也拦(锚在 home 的规则接不住);库内 _Private/ 有下划线,不命中
  { name: '~/Private 相对路径形态', re: /(?:^|[\s'"(])(?:\.\/)?Private\// },
  // 标讯 skill 的 key 文件(~/.zlbx/:config.json 与 env.sh;2026-09-03 从 ~/.claude/settings.json 的 env 挪出来,读 settings 不再带出 key)
  { name: 'zlbx API key(~/.zlbx)', re: /(?:~|\$\{?HOME\}?|\/home\/[\w.-]+)\/\.zlbx(?:\/|$|["'\s])/ },
];
...
for (const raw of targets) {
  ...
  for (const r of RULES) {
    ...
    console.error(`🔒 protect-secrets:命中「${r.name}」,已阻断 ${tool}。密钥永远在库外、不进上下文(见 navigation/基建.md);确需查看请用户人工开终端。误拦则改 brain-kit lib/hooks/protect-secrets.js 的 RULES 并跑 npm test。`);
    process.exit(2);
  }
}
process.exit(0);
