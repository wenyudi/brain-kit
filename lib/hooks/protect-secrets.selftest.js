...
const CASES = [
  // —— 必须拦 ——
  ['block', 'Bash', { command: 'cat ~/.zlbx/config.json' }],
  ['block', 'Read', { file_path: '/home/raul/.zlbx/env.sh' }],
  ['allow', 'Bash', { command: 'ls ~/.zlbxx' }],
  ['block', 'Read', { file_path: '/home/raul/.claude/.credentials.json' }],
  ['block', 'Bash', { command: 'cat ~/.codex/auth.json' }],
  ['block', 'Bash', { command: 'cat .netrc' }],
  ['block', 'Bash', { command: 'cd ~ && cat Private/key.md' }],
  ['allow', 'Read', { file_path: '/mnt/workspace/Raul/Obsidian Vault/_Private/自建网络服务清单.md' }],
  ['allow', 'Bash', { command: 'cat ~/.claude/settings.json' }],
