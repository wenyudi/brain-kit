...
const HUMAN = ['pages', 'digest', 'navigation', '_Private', '_system'];
...
try {
  ...
  if (hc.enabled) { const un = S.unharvested(hc); if (un.length) P.sediment.push(`未捞会话 ${un.length} 场（夜间 sweep 每晚 ${hc.nightlyCap} 场；node brain-tools/sessions.js list）`); }
