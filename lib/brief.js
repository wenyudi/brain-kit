...
try {
  ...
  if (clips) lines.push(`📥 inbox 剪藏 ${clips} 条未入库——跑 \`node brain-tools/ingest.js --no-asr\` 机械搬进藏书房（无字幕 B 站留给夜里 ASR）`);
...
try {
  ...
  if (hc.enabled) {
    ...
    if (un.length) lines.push(`🪣 未捞会话 ${un.length} 场（夜间自动捞每晚 ${hc.nightlyCap} 场；现在捞就说「捞 #n」，路径见 \`node brain-tools/sessions.js list\`）：` +
